...
var vecBuiltinStringCases = map[string][]vecExprBenchCase{
	...
	ast.Substring: {
		...
		},
		{
			...
			childrenFieldTypes: []*types.FieldType{
				types.NewFieldTypeBuilderP().SetType(mysql.TypeString).SetFlag(mysql.BinaryFlag).SetCharset(charset.CharsetBinary).SetCollate(charset.CollationBin).BuildP(),
	...
	ast.Locate: {
		...
		},
		{
			...
			childrenTypes: []types.EvalType{types.ETString, types.ETString},
			childrenFieldTypes: []*types.FieldType{
				types.NewFieldTypeBuilderP().SetType(mysql.TypeString).SetFlag(mysql.BinaryFlag).SetCharset(charset.CharsetBinary).SetCollate(charset.CollationBin).BuildP(), nil},
			geners: []dataGenerator{newRandLenStrGener(0, 10), newRandLenStrGener(0, 20)},
		},
		{
			retEvalType:   types.ETInt,
		...
		},
		{
			...
			childrenFieldTypes: []*types.FieldType{
				types.NewFieldTypeBuilderP().SetType(mysql.TypeString).SetFlag(mysql.BinaryFlag).SetCharset(charset.CharsetBinary).SetCollate(charset.CollationBin).BuildP(),
				types.NewFieldTypeBuilderP().SetType(mysql.TypeString).SetFlag(mysql.BinaryFlag).SetCharset(charset.CharsetBinary).SetCollate(charset.CollationBin).BuildP(),
				types.NewFieldTypeBuilderP().SetType(mysql.TypeInt24).BuildP()},
			geners: []dataGenerator{newRandLenStrGener(0, 10), newRandLenStrGener(0, 20), newRangeInt64Gener(-10, 20)},
		},
		{
			retEvalType:   types.ETInt,
