...
func TestBootstrapWithError(t *testing.T) {
	...
	_, err = exec(se, "SELECT * from mysql.tidb_agent_memory_profile_version")
	require.ErrorContains(t, err, "AGENT_MEMORY_TENANT_CONTEXT")
	_, err = exec(se, "SELECT * from mysql.tidb_agent_memory_audit")
	require.ErrorContains(t, err, "AGENT_MEMORY_TENANT_CONTEXT")
	_, err = exec(se, "SELECT COUNT(*) FROM mysql.agent_memory_all")
	require.ErrorContains(t, err, "AGENT_MEMORY_TENANT_CONTEXT")
	MustExec(t, se, "SET @@tidb_agent_tenant_id='__missing__'")
	MustExec(t, se, "SET @@tidb_agent_namespace='__missing__'")
	r = MustExecToRecodeSet(t, se, "SELECT COUNT(*) FROM mysql.tidb_agent_memory_audit WHERE action='policy_denied'")
	req = r.NewChunk(nil)
	err = r.Next(ctx, req)
	require.NoError(t, err)
	require.GreaterOrEqual(t, req.GetRow(0).GetInt64(0), int64(3))
	require.NoError(t, r.Close())
	MustExec(t, se, "SET @@tidb_agent_tenant_id='default'")
	MustExec(t, se, "SET @@tidb_agent_namespace='default'")
	...
	MustExec(t, se, "INSERT INTO mysql.tidb_agent_memory_audit (tenant_id, namespace, actor, action, object_type, object_id, reason) VALUES ('tenant_a', 'ns_a', 'agent_a', 'read', 'memory', 'm_a', 'allowed')")
	MustExec(t, se, "INSERT INTO mysql.tidb_agent_memory_audit (tenant_id, namespace, actor, action, object_type, object_id, reason) VALUES ('tenant_b', 'ns_b', 'agent_b', 'policy_denied', 'memory', 'm_b', 'missing_context')")
	...
	require.NoError(t, r.Close())
	r = MustExecToRecodeSet(t, se, "SELECT COUNT(*) FROM mysql.tidb_agent_memory_episodic")
	req = r.NewChunk(nil)
	err = r.Next(ctx, req)
	require.NoError(t, err)
	require.Equal(t, int64(1), req.GetRow(0).GetInt64(0))
	require.NoError(t, r.Close())
	r = MustExecToRecodeSet(t, se, "SELECT tenant_id FROM mysql.tidb_agent_memory_episodic WHERE memory_id = 2")
	req = r.NewChunk(nil)
	err = r.Next(ctx, req)
	require.NoError(t, err)
	require.Equal(t, 0, req.NumRows())
	require.NoError(t, r.Close())
	r = MustExecToRecodeSet(t, se, "SELECT COUNT(*) FROM mysql.agent_memory_all WHERE tenant_id='tenant_b'")
	req = r.NewChunk(nil)
	err = r.Next(ctx, req)
	require.NoError(t, err)
	...
	require.NoError(t, r.Close())
	r = MustExecToRecodeSet(t, se, "SELECT COUNT(*) FROM mysql.tidb_agent_memory_audit WHERE reason='allowed'")
	req = r.NewChunk(nil)
	err = r.Next(ctx, req)
	require.NoError(t, err)
	require.Equal(t, int64(1), req.GetRow(0).GetInt64(0))
	require.NoError(t, r.Close())
	r = MustExecToRecodeSet(t, se, "SELECT COUNT(*) FROM mysql.tidb_agent_memory_audit WHERE actor='agent_b'")
	req = r.NewChunk(nil)
	err = r.Next(ctx, req)
	require.NoError(t, err)
	require.Equal(t, int64(0), req.GetRow(0).GetInt64(0))
	require.NoError(t, r.Close())
	r = MustExecToRecodeSet(t, se, "SELECT COUNT(*) FROM mysql.tidb_agent_memory_audit WHERE action='read'")
	req = r.NewChunk(nil)
	err = r.Next(ctx, req)
	require.NoError(t, err)
	require.GreaterOrEqual(t, req.GetRow(0).GetInt64(0), int64(1))
	require.NoError(t, r.Close())
	r = MustExecToRecodeSet(t, se, "SELECT COUNT(*) FROM mysql.tidb_agent_memory_audit WHERE action='write'")
	req = r.NewChunk(nil)
	err = r.Next(ctx, req)
	require.NoError(t, err)
	require.GreaterOrEqual(t, req.GetRow(0).GetInt64(0), int64(1))
	...
	require.Equal(t, int64(2), req.GetRow(0).GetInt64(0))
	require.NoError(t, r.Close())
	MustExec(t, se, "DELETE FROM mysql.tidb_agent_memory_procedural WHERE subject_id='subject_a'")
	r = MustExecToRecodeSet(t, se, "SELECT COUNT(*) FROM mysql.tidb_agent_memory_audit WHERE action='delete' AND object_id='mysql.tidb_agent_memory_procedural'")
	req = r.NewChunk(nil)
	err = r.Next(ctx, req)
	require.NoError(t, err)
	require.GreaterOrEqual(t, req.GetRow(0).GetInt64(0), int64(1))
...
func TestUpgrade(t *testing.T) {
	...
	_, err = exec(se2, "SELECT * FROM mysql.tidb_agent_memory_profile_version")
	require.ErrorContains(t, err, "AGENT_MEMORY_TENANT_CONTEXT")
	_, err = exec(se2, "SELECT * FROM mysql.tidb_agent_memory_audit")
	require.ErrorContains(t, err, "AGENT_MEMORY_TENANT_CONTEXT")
	_, err = exec(se2, "SELECT * FROM mysql.agent_memory_all")
	require.ErrorContains(t, err, "AGENT_MEMORY_TENANT_CONTEXT")
	MustExec(t, se2, "SET @@tidb_agent_tenant_id='__missing__'")
	MustExec(t, se2, "SET @@tidb_agent_namespace='__missing__'")
	r = MustExecToRecodeSet(t, se2, "SELECT COUNT(*) FROM mysql.tidb_agent_memory_audit WHERE action='policy_denied'")
	req = r.NewChunk(nil)
	err = r.Next(ctx, req)
	require.NoError(t, err)
	require.GreaterOrEqual(t, req.GetRow(0).GetInt64(0), int64(3))
	require.NoError(t, r.Close())
	MustExec(t, se2, "SET @@tidb_agent_tenant_id='default'")
	MustExec(t, se2, "SET @@tidb_agent_namespace='default'")
