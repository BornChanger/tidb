...
func TestBootstrapWithError(t *testing.T) {
	...
	require.NoError(t, r.Close())
	r = MustExecToRecodeSet(t, se, "SELECT COUNT(*) FROM mysql.tidb_agent_memory_audit")
	req = r.NewChunk(nil)
	err = r.Next(ctx, req)
	require.NoError(t, err)
	require.Equal(t, int64(1), req.GetRow(0).GetInt64(0))
	require.NoError(t, r.Close())
	r = MustExecToRecodeSet(t, se, "SELECT actor FROM mysql.tidb_agent_memory_audit WHERE audit_id = 2")
	req = r.NewChunk(nil)
	err = r.Next(ctx, req)
	require.NoError(t, err)
	require.Equal(t, 0, req.NumRows())
