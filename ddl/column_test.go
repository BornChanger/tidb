...
func (s *testColumnSuiteToVerify) TestAutoConvertBlobTypeByLength() {
	...
	require.Equal(s.T(), t.Cols()[0].GetType(), mysql.TypeTinyBlob)
	require.Equal(s.T(), t.Cols()[0].GetFlen(), tinyBlobMaxLength)
	require.Equal(s.T(), t.Cols()[1].GetType(), mysql.TypeBlob)
	require.Equal(s.T(), t.Cols()[1].GetFlen(), blobMaxLength)
	require.Equal(s.T(), t.Cols()[2].GetType(), mysql.TypeMediumBlob)
	require.Equal(s.T(), t.Cols()[2].GetFlen(), mediumBlobMaxLength)
	require.Equal(s.T(), t.Cols()[3].GetType(), mysql.TypeLongBlob)
	require.Equal(s.T(), t.Cols()[3].GetFlen(), longBlobMaxLength)
