...
func CheckModifyTypeCompatible(origin *types.FieldType, to *types.FieldType) (canReorg bool, err error) {
	...
	stringToString := types.IsString(origin.GetType()) && types.IsString(to.GetType())
...
func needReorgToChange(origin *types.FieldType, to *types.FieldType) (needReorg bool, reasonMsg string) {
	...
	if toFlen > 0 && toFlen != originFlen {
		...
		isBinaryType := func(tp *types.FieldType) bool { return tp.GetType() == mysql.TypeString && types.IsBinaryStr(tp) }
...
func checkTypeChangeSupported(origin *types.FieldType, to *types.FieldType) bool {
	if (types.IsTypeTime(origin.GetType()) || origin.GetType() == mysql.TypeDuration || origin.GetType() == mysql.TypeYear ||
		types.IsString(origin.GetType()) || origin.GetType() == mysql.TypeJSON) &&
...
func (d *ddl) getModifiableColumnJob(ctx context.Context, sctx sessionctx.Context, ident ast.Ident, originalColName model.CIStr,
	...
	if err = checkModifyTypes(sctx, &col.FieldType, &newCol.FieldType, isColumnWithIndex(col.Name.L, t.Meta().Indices)); err != nil {
		if strings.Contains(err.Error(), "Unsupported modifying collation") {
			...
			err = errUnsupportedModifyCollation.GenWithStack(colErrMsg, col.Name.L, col.GetCollate(), newCol.GetCollate())
		}
		return nil, errors.Trace(err)
	}
...
func checkAutoRandom(tableInfo *model.TableInfo, originCol *table.Column, specNewColumn *ast.ColumnDef) (uint64, error) {
	...
	if modifyingAutoRandCol {
		...
		if originCol.GetType() != mysql.TypeLonglong {
			return 0, ErrInvalidAutoRandom.GenWithStackByArgs(fmt.Sprintf(autoid.AutoRandomOnNonBigIntColumn, types.TypeStr(originCol.GetType())))
		}
...
func (d *ddl) AlterTableSetTiFlashReplica(ctx sessionctx.Context, ident ast.Ident, replicaInfo *ast.TiFlashReplicaSpec) error {
	...
	for _, col := range tb.Cols() {
		...
		if !ok {
			return errAlterReplicaForUnsupportedCharsetTable.GenWithStackByArgs(col.GetCharset())
		}
	}
	...
	for _, col := range tblInfo.Columns {
		...
		if err = checkModifyCharsetAndCollation(toCharset, toCollate, col.GetCharset(), col.GetCollate(), isColumnWithIndex(col.Name.L, tblInfo.Indices)); err != nil {
			if strings.Contains(err.Error(), "Unsupported modifying collation") {
				...
				err = errUnsupportedModifyCollation.GenWithStack(colErrMsg, col.Name.L, col.GetCollate(), toCollate)
			}
			return doNothing, err
		}
	}
