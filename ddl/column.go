...
func (w *worker) onModifyColumn(d *ddlCtx, t *meta.Meta, job *model.Job) (ver int64, _ error) {
	...
	if jobParam.changingCol == nil {
		...
		for i, idxInfo := range idxInfos {
			...
			canPrefix := types.IsTypePrefixable(jobParam.changingCol.GetType())
...
func generateOriginDefaultValue(col *model.ColumnInfo) (interface{}, error) {
	...
	if odValue == strings.ToUpper(ast.CurrentTimestamp) {
		if col.GetType() == mysql.TypeTimestamp {
			odValue = types.NewTime(types.FromGoTime(time.Now().UTC()), col.GetType(), col.GetDecimal()).String()
		} else if col.GetType() == mysql.TypeDatetime {
			odValue = types.NewTime(types.FromGoTime(time.Now()), col.GetType(), col.GetDecimal()).String()
		}
	}
	return odValue, nil
}
